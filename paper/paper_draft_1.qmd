...
# Merge the two VIF data frames by "Variable", adding suffixes to differentiate columns
combined_vif_df <- merge(harris_vif_df, trump_vif_df, by = "Variable", suffixes = c("_Harris", "_Trump"))
...
Appendix: Ideal Methodology and Survey for Predicting the U.S. Presidential Election
...
#| echo: false
#| label: tbl-dkpv
#| tbl-cap: "Descriptions of Key Polling Variables"

library(knitr)

# Data frame with selected variables and descriptions only
variable_descriptions <- data.frame(
  Variable = c("poll_id", "pollster_id", "pollster", "sponsor_ids", "pollscore", "methodology", 
               "transparency_score", "state", "start_date", "end_date", "sample_size", 
               "population", "created_at", "ranked_choice_reallocated", "hypothetical"),
  Description = c(
    "Unique identifier for each poll conducted.",
    "Unique identifier for the polling organization.",
    "Name of the polling organization.",
    "Unique identifier(s) for the sponsor(s).",
    "A numeric value representing pollster reliability score.",
    "Method used to conduct the poll.",
    "Score reflecting pollster’s transparency.",
    "State where poll was conducted or focused.",
    "Date when the poll started.",
    "Date when the poll ended.",
    "Total number of respondents.",
    "Description of respondent group.",
    "Timestamp when poll data was created.",
    "Indicates if ranked-choice reallocations were applied.",
    "Indicates if poll is about a hypothetical match-up."
  )
)

# Display the table with only Variable and Description columns
kable(variable_descriptions)
