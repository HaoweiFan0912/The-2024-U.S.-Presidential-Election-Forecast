...
```{r, fig.pos="H", fig.height=10, fig.width=20}
#| warning: false
#| message: false
#| echo: false
#| label: fig-doditd
...
plots[["Trump"]]<-ggplot(train_Trump, aes(x = duration)) +
  geom_histogram(binwidth = 2, fill = "blue", color = "black", alpha = 0.7) +
  labs(title = "Histogram of duration for Trump", x = "Duration", y = "Frequency")
...
### Combine and Print Plots ###
# Combine all individual plots into one using patchwork with 2 columns and 4 rows
combined_plot <- wrap_plots(plots, ncol = 2, nrow = 1, heights = unit(rep(5, 5), "in")) +
  plot_annotation(
    theme = theme(
      plot.title = element_text(hjust = 0.5, size = 14)
    )
