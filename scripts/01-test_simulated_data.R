...
analysis_data_3$numeric_grade <- pmin(pmax(analysis_data_3$numeric_grade, 1), 5)
...
analysis_data_3$transparency_score <- pmin(pmax(analysis_data_3$transparency_score, 1), 10)
...
test_that("Simulated datasets contain all expected columns", {
  ...
  # Specific check for `poll_id` in analysis_data_3
  expect_true("poll_id" %in% colnames(analysis_data_3), 
              info = "`poll_id` is missing in analysis_data_3.")
})
