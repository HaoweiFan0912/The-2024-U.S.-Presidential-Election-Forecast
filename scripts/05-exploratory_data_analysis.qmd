...
hypothetical_ratio <- table(simulated_candidate_1$hypothetical)
...
```{r}
# Find the maximum difference between start_date and end_date
date_diff <- simulated_candidate_1 %>%
  mutate(date_difference = as.numeric(difftime(end_date, start_date, units = "days"))) %>%
  summarise(max_difference = max(date_difference))
